 ...
 * until the user kits [Enter] on the keyboard.
 *
...
private:
  ...
  /**
   * @brief displayItemDetails Output the detailed consumeable item info to
  ...
  /**
   * @brief pickupItem Pick-up the item in the room, if any, and assign it
   * to the player. Prompts for user confirmation if the player already has an item.
   */
  void pickupItem();

  /**
   * @brief compareItems Display the player's current item and the item in the room if any.
   */
  void compareItems();
  ...
  /**
   * @brief useItem Have the player use its item, if any
   */
  void useItem();
